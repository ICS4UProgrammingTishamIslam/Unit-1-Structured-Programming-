	...
	public static String StringBlowup(String str) {
		...
		//if there is a number preceding (char adder would equal 0 otherwise), add that many letters to the final string
			...
			if (Character.isDigit(blowing[letter])) {
				//Swear to god this took me too long. Convert the char value into an number or you'll feel great emotional pain
				//because for some reason java gives the ASCII values for numbers in character form. not helpful 0/10
				charAdder = Character.getNumericValue(blowing[letter]);
			}
